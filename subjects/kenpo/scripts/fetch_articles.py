"""Fetch 日本国憲法 (Constitution) article texts from the e-Gov Law API.

Reads data/curation.json for the list of needed article numbers, fetches
each one's plain text from e-Gov, and writes data/articles_raw.json.
The special id "zenbun" (前文) is fetched from the lawdata endpoint since
the preamble has no article number.
"""
import json
import time
...
LAW_ID = "321CONSTITUTION"  # 日本国憲法
API_BASE = "https://laws.e-gov.go.jp/api/1/articles"
LAWDATA_BASE = "https://laws.e-gov.go.jp/api/1/lawdata"
...
def fetch_article(article_number):
    ...
    title, body = format_article(article_element)
    return title, body


def fetch_preamble():
    url = f"{LAWDATA_BASE}/{LAW_ID}"
    with urllib.request.urlopen(url, timeout=10) as resp:
        xml_text = resp.read()
    root = ET.fromstring(xml_text)
    code = root.findtext("Result/Code")
    if code != "0":
        raise RuntimeError(f"lawdata: e-Gov returned code {code}")
    preamble = root.find(".//Preamble")
    if preamble is None:
        raise RuntimeError("lawdata: no Preamble element in response")
    body = "\n".join(format_paragraph(p) for p in preamble.findall("Paragraph"))
    return "前文", body
...
def main():
    ...
    for i, num in enumerate(numbers, 1):
        try:
            if num == "zenbun":
                title, body = fetch_preamble()
            else:
                title, body = fetch_article(num)
            results[num] = {"title": title, "text": body}
            print(f"[{i}/{len(numbers)}] OK {num} {title}")
