"""Fetch 民法 (Civil Code) article texts from the e-Gov Law API.
...
def main():
    ...
    for i, num in enumerate(numbers, 1):
        try:
            ...
            print(f"[{i}/{len(numbers)}] OK 第{num}条 {title}")
        except Exception as exc:  # noqa: BLE001
            errors.append((num, str(exc)))
            print(f"[{i}/{len(numbers)}] FAIL 第{num}条: {exc}")
        time.sleep(0.15)
